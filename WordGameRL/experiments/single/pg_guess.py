...
from experiments.utils import (CURRENT_TIME, deterministic_guessers,
                               play_a_word, test_on_words, topic_guesseres)
from src.agents.bert_agent import ActorCriticAgent, PGACAgent


def get_immediate_rewards(player, clues, guesses, topics=None, target=None):
    rewards = []
    for i in range(len(clues)):
        prev_clues = clues[:i]
        prev_guesses = guesses[:i]
        action = clues[i]
        reward = player.get_probability_gain(prev_clues, prev_guesses, action, 
                                                guesser_topics=prev_guesses)
        reward += 1
        if prev_guesses:
            prop = 0
            state = (player.target, prev_clues, prev_guesses)
            attentions = player.model.get_input_gradient(state, action)
            
            for guess in prev_guesses:
                prop += 2 * attentions[guess]
            reward = 0.7 * reward + 0.3 * prop
...
def add_experience(giver, guesser, words):
    ...
    if isinstance(giver, PGACAgent):
        rewards = get_immediate_rewards(giver, clues, guesses, topics=guesser.topics)
...
def main():
    ...
    res_dir = "./single_pg_guess_res"
    if not os.path.exists(res_dir):
        os.mkdir(res_dir)
    path = 'single_pg_guess_{0}.json'.format(CURRENT_TIME)
    ...
    for epoch in range(epochs):
        ...
        for num in range(one_epoch_word_num):
            guesser = random.choice(topic_guesseres)
