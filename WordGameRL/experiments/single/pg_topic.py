...
def get_immediate_rewards(player, clues, guesses, topics=None, target=None):
    ...
    for i in range(len(clues)):
        ...
        action = clues[i]
        reward = player.get_probability_gain(prev_clues,  prev_guesses, action, guesser_topics=topics)
...
def main():
    ...
    res_dir = "./single_pg_topic_res"
    if not os.path.exists(res_dir):
        os.mkdir(res_dir)
    path = 'single_pg_topic_{0}.json'.format(CURRENT_TIME)
